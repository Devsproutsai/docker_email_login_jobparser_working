...
linkedin_url = os.getenv("linkedin_url", "https://www.linkedin.com/in/dhanunjayam/")


class Test_resume_parser_cases:
    ...
    @pytest.mark.usefixtures("browser")
    def test_resume_parser_add_candidate_using_url(self, browser):
        browser.get(home_page)
        # Select the element by its text content
        element = WebDriverWait(browser, 20).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '.cursor-pointer:nth-child(2) svg'))
        ...
        add_candidate_url = WebDriverWait(browser, 10).until(
                    EC.element_to_be_clickable((By.XPATH, "//*[text()='Add using URL']")))

        # Click the button
        add_candidate_url.click()

        time.sleep(3)
        enter_url = browser.find_element(By.XPATH, '//input[@value=""]')

        enter_url.send_keys(linkedin_url)

        Add_button = WebDriverWait(browser, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//*[text()='Add']")))
        Add_button.click()
    ...
    @pytest.mark.usefixtures("browser")
    def test_resume_parser_retrieve(self, browser):
        browser.get(home_page)
        # Select the element by its text content
        element = WebDriverWait(browser, 20).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, '.cursor-pointer:nth-child(2) svg'))
        ...
        Retrieve = WebDriverWait(browser, 10).until(
                    EC.element_to_be_clickable((By.XPATH, "//*[text()='Retrieve']")))
