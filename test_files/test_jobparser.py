...
class Test_job_parser_cases:
    ...
    @pytest.mark.usefixtures("browser")
    def test_create_new_jd(self,browser):
        wait = WebDriverWait(browser, 10)
        ...
        #review post
        # 34 | click | id=review | 
        wait.until(EC.element_to_be_clickable((By.XPATH, "//*[text()='Review post']"))).click()

        #publish this job
        # 35 | click | css=.next | 
        wait.until(EC.element_to_be_clickable((By.XPATH, "//*[text()='Publish this job']"))).click()
        
        time.sleep(5)
        
        assert "Job created successfully" in browser.page_source, "ERROR in the generated JD"

        view = WebDriverWait(browser, 15).until(
                    EC.visibility_of_element_located((By.XPATH, '//button[text()="View"]')))
