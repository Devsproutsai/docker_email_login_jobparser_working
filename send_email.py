...
devemail = os.getenv("RECEIVER_EMAIL", "dev@sproutsai.com")
personalemail = os.getenv("RECEIVER_EMAIL1", "tvsdhanan009@gmail.com")

def send_email(subject, body, attachment_paths,receiver_emails):
    ...
    with smtplib.SMTP_SSL(smtp_server, 465) as server:
        ...
        server.sendmail(from_email, receiver_emails, message.as_string())

    print(f"Email sent to {receiver_emails} with attachments.")
...
def analyze_test_report():
    ...
    \nList of Tests cases automated :
    \n1. Login with correct username and password and logout successfully
    \n2. Login with correct username and wrong password
    \n3. Login with wrong username and correct password
    \n4. Login with wrong username and wrong password
    \n5. Posting a new job with an empty docx format job description
    \n6. Posting a new job with an empty pdf format job description
    \n7. Posting a new job with an empty txt format job description
    \n8. Posting a new job with a normal docx format job description
    \n9. Posting a new job with a normal pdf format job description
    \n10. Posting a new job with a normal txt format job description
    \n11. Generating a new job description
    \n12. Posting a new job with a normal docx format and uploading a single resume to the job description
    \n13. Posting a new job with a normal docx format and uploading multiple resumes to the job description
    \n14. Uploading a single resume to the first job title available
    \n15. Uploading multiple resumes to the first job title available
    \n16. Uploading a single resume to the custom job title
    \n17. Uploading multiple resumes to the custom job title
    \n18. Adding candidate using the linkedin url
    \n19. Adding candidates using the Retrieve
    """
    ...
    send_email('Test Report', email_body, attachments,[devemail, personalemail])
